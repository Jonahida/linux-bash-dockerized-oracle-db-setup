    ...
    if ! command -v git &> /dev/null; then
        ...
        if [[ $choice =~ ^[Yy]$ ]]; then
            ...
            if [[ "$(uname)" == "Darwin" ]]; then
                brew install git || { show_error "Failed to install Git using Homebrew. Please install Git manually."; exit 1; }
            elif [[ "$(expr substr $(uname -s) 1 5)" == "Linux" ]]; then
                sudo apt-get install git || { show_error "Failed to install Git using apt-get. Please install Git manually."; exit 1; }
            else
...
move_downloaded_file() {
    ...
    if [[ $move_choice =~ ^[Yy]$ ]]; then
        ...
        if [ -f "$download_path" ]; then
            ...
            mv "$download_path" "$target_path" || { show_error "Failed to move the file. Please move it manually."; exit 1; }
...
check_docker_image_exists || { exit 1; }
...
prompt_oracle_db_version || { exit 1; }

# Move the downloaded file if the user agrees
move_downloaded_file || { exit 1; }

# Execute the build script with the specified Oracle DB version
docker-images/OracleDatabase/SingleInstance/dockerfiles/buildContainerImage.sh -v "$db_version" -o --network=host -e -t oracle19c || { exit 1; } 
